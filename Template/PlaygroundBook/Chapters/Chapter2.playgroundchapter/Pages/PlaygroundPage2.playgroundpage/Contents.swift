...
//: - Note:
//: Add the method `nullifyBallsVelocity()` right above `detectCollisions()` to assign a null vector to the ball's velocity and click on run code to see what happens.
//:
detectCollisions()
